...
export async function proxy(request: NextRequest) {
  ...
  // Local visual-review mode has no reachable Supabase auth service. Skipping
  // the refresh here keeps public reservation requests instant; production can
  // never enter this branch.
  const previewMode =
    process.env.NODE_ENV !== 'production' &&
    (process.env.RESERVATION_PREVIEW_MODE === '1' ||
      process.env.RESERVATION_PREVIEW_MODE === 'true');
  if (previewMode) return response;
