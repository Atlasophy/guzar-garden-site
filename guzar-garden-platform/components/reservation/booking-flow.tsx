...
export function BookingFlow({
  ...
  const loadAvailability = useCallback(
    async (nextDate: string, nextPartySize: number, nextTime?: string | null) => {
      ...
      if (!result.ok) {
        setError({
          code: result.code,
          message: result.code === 'network' ? dictionary.errors.network : dictionary.errors.generic,
        });
        return null;
      }
  ...
  const stepValues: Record<Step, string> = {
    ...
    date: date ? formatLocalDate(new Date(`${date}T12:00:00Z`), locale, { day: 'numeric', month: 'short' }) : '—',
    time: time ?? '—',
      ...
      <div>
        ...
        {step === 'time' ? (
          <section className="panel" aria-labelledby="step-time">
            ...
            {!loading && availability && availability.slots.length === 0 && availability.state !== 'closed_that_day' ? (
              <div className="notice warn">
                <div>
                  <b>{t.timeNone}</b>
        ...
        {step === 'table' ? (
          <section className="panel" aria-labelledby="step-table">
            ...
            <p className="hint">{t.tableHint}</p>
