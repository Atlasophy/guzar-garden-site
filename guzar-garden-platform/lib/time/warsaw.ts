...
export function zonedTimeToInstant(
  ...
  // Guess with the offset in force at the naive timestamp, then correct with the
  // offset actually in force at the guessed instant.
  const guessOffset = zoneOffsetMs(new Date(naive), timeZone);
  const firstGuess = naive - guessOffset;
  const correctedOffset = zoneOffsetMs(new Date(firstGuess), timeZone);
  const candidate = naive - correctedOffset;

  const roundTrip = toZonedParts(new Date(candidate), timeZone);
  const matches =
    roundTrip.year === year &&
  ...
  if (!matches) {
    ...
    // useful answer is the first instant after the clocks moved.
    return { instant: new Date(naive - guessOffset), kind: 'skipped' };
  }

  if (guessOffset !== correctedOffset) {
    // Two offsets both produce this wall time: the hour repeated in autumn.
    const alternative = naive - guessOffset;
    const alternativeParts = toZonedParts(new Date(alternative), timeZone);
    const alternativeMatches =
      alternativeParts.year === year &&
      alternativeParts.month === month &&
      alternativeParts.day === day &&
      alternativeParts.hour === hour &&
      alternativeParts.minute === minute;
    if (alternativeMatches) {
      return { instant: new Date(Math.min(candidate, alternative)), kind: 'ambiguous' };
    }
  }

  return { instant: new Date(candidate), kind: 'exact' };
}
