...
export interface StaffFloorView {
  at: string;
  localTime: string;
  areas: { id: string; slug: string; name: string; x: number; z: number; width: number; depth: number }[];
...
export async function getFloorView(
  ...
  for (const allocation of allocations ?? []) allocationByTable.set(allocation.table_id, allocation);
